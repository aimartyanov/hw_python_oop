...
class Running(Training):
    ...
    def get_spent_calories(self) -> float:
        calories = (self.coef1 * self.get_mean_speed() - self.coef2) * self.weight / self.M_IN_KM * (
...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        calories = (self.coefw1 * self.weight + (
                self.get_mean_speed() ** 2 // self.height) * self.coefw2 * self.weight) * (
                           self.duration * self.M_IN_H)
        return calories
...
class Swimming(Training):
    ...
    def get_spent_calories(self) -> float:
        calories = (Swimming.get_mean_speed(self) + self.coefs1) * self.coefs2 * self.weight
        return calories
